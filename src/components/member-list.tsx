...
export const projects: Project[] = [
  {
    id: "1",
    name: "LNDev UI - Ga-Rankuwa Components",
    status: status[0],
    icon: "Cuboid",
    percentComplete: 80,
...
export const teams: Team[] = [
  {
    ...
    joined: true,
    color: "#FF0000",
    Students: [users[8], users[10], users[2], users[3], users[4]],
...
function TeamsTooltip({ teamId }: TeamsTooltipProps) {
  return (
    <Tooltip delayDuration={0}>
      ...
      <TooltipContent className="p-2">
        <div className="flex flex-col gap-1">
          {teams
            .filter((team) => teamId.includes(team.id))
            .map((team) => (
...
function StudentLine({ user }: StudentLineProps) {
  return (
    <div className="w-full flex items-center py-3 px-6 border-b hover:bg-sidebar/50 border-muted-foreground/5 text-sm last:border-b-0">
      ...
      <div className="w-40 shrink-0 flex text-xs text-muted-foreground">
        <TeamsTooltip teamId={user.teamIds} />
      </div>
    </div>
  );
}

const StudentList = React.forwardRef<
  HTMLDivElement,
...
>(({ className, ...props }, ref) => (
  ...
  >
    ...
    <div className="w-full">
      {users.map((user) => (
        <StudentLine key={user.id} user={user} />
      ))}
    </div>
  </div>
));
